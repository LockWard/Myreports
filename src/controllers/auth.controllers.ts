...
export const signUp = async (req: Request, res: Response): Promise<Response | any> => {
    const { user_name, user_email, user_password, department_name } = req.body
    try {
        ...
        if (result.length) {
            ...
            const result = await userModels.postUser(user_name, user_email, hashedPassword, department_name)

            return res.status(200).json({ 
                message: `User '${user_name}' created. Token: ` + 
                createToken(result.insertId as unknown as string)
            })
        }
...
export const signIn = async (req: Request, res: Response): Promise<Response | any> => {
    ...
    try {
        ...
        if(!passwordMatch){
            ...
            return res.status(200).json({ msg: 'Login successful. Token: ' + createToken(user.user_id) });
        }
    } catch (error) {
